...
FcMpComponent<FcMpViewContextBase & FcMpViewContextAny>(
    createLiaisonMixin(MpViewType.Component, "fc-api-detail"),
    {
        properties: {
            data: {
                type: String,
            ...
            this.$fcOn(
                ...
                (type, data: FcMpDispatchEventData) => {
                    if (data.child.tid === "fc-collapse") {
                        const tidAlias = data.child.tidAlias as string;
                        if (tidAlias.startsWith("ApiCollapse")) {
                            const target = tidAlias.substr(
                                "ApiCollapse".length
                            );
                            type = data.type;
                            if (type === "toggle") {
                        ...
                        const tidAlias = data.child.tidAlias;
